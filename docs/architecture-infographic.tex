...
\usepackage{xcolor}
% smallest text size that stays above the 6.5 pt rendered-bbox floor
\def\fsxs{\fontsize{6.6}{8.4}\selectfont}
\usepackage{tikz}
...
\tikzset{
  ...
  stack item/.style={
    ...
    font=\rmfamily\footnotesize,text=Ink,text width=43mm
  },
  ...
  pipe node/.style={
    ...
    font=\rmfamily\footnotesize\bfseries,text=Ink,text width=31mm,minimum height=11mm
  },
  ...
  gate node/.style={
    ...
    font=\rmfamily\scriptsize\bfseries,text=Coral!90!black,align=center
  },
  % arrows
  primary flow/.style={
  ...
  edge label/.style={
    fill=Paper,inner sep=1.5pt,font=\rmfamily\scriptsize,text=Slate,align=center
  },
  % metric
  metric/.style={
...
\begin{tikzpicture}[x=1mm,y=1mm]
  ...
  \node[metric,fill=Ocean,anchor=north west] (m1) at (4,-20) {
    {\Large 12\quad}{\footnotesize 个场景}\\[.4mm]
    {\fsxs\color{white!85}分析问题全覆盖}
  };
  \node[metric,fill=Teal,anchor=north west] (m2) at (47,-20) {
    {\Large 4.06\quad}{\footnotesize M 行}\\[.4mm]
    {\fsxs\color{white!85}总合成数据量}
  };
  \node[metric,fill=Gold,anchor=north west] (m3) at (90,-20) {
    {\Large 0\quad}{\footnotesize 外部请求}\\[.4mm]
    {\fsxs\color{white!85}完全离线运行}
  };
  \node[metric,fill=Violet,anchor=north west] (m4) at (133,-20) {
    {\Large 3\quad}{\footnotesize 重验证门}\\[.4mm]
    {\fsxs\color{white!85}透明 / 墨迹 / 饱和}
  };
  ...
  \begin{pgfonlayer}{edge layer}
    ...
    \draw[data flow] (ml1.east) -- (vl1.west)
      node[edge label,midway,above=.3mm] {聚合\\结果};
  ...
  \node[scene card,fill=Magma!8] (c1) at (4,-125) {
    {\ttfamily\fsxs\bfseries\color{Magma} density}\hfill{\fsxs\color{Slate}1,000,000 行}\\[.5mm]
    {\bfseries\scriptsize 百万点密度热图}\\[.2mm]
    {\scriptsize\color{Slate}屏幕分辨率下，一百万点集中在哪？}
  };
  \node[scene card,fill=Ocean!8] (c2) at (62,-125) {
    {\ttfamily\fsxs\bfseries\color{Ocean} signals}\hfill{\fsxs\color{Slate}400,000 行}\\[.5mm]
    {\bfseries\scriptsize 密集时序信号图集}\\[.2mm]
    {\scriptsize\color{Slate}四条长时序如何保持可探索性？}
  };
  \node[scene card,fill=Teal!8] (c3) at (120,-125) {
    {\ttfamily\fsxs\bfseries\color{Teal} linked}\hfill{\fsxs\color{Slate}320,000 行}\\[.5mm]
  ...
  \node[scene card,fill=Gold!12] (c4) at (4,-144) {
    {\ttfamily\fsxs\bfseries\color{Gold!70!black} distribution}\hfill{\fsxs\color{Slate}360,000 行}\\[.5mm]
    {\bfseries\scriptsize 数据库驱动分布对比}\\[.2mm]
    {\scriptsize\color{Slate}四个队列在密集测量上有何差异？}
  };
  \node[scene card,fill=Violet!8] (c5) at (62,-144) {
    {\ttfamily\fsxs\bfseries\color{Violet} regression}\hfill{\fsxs\color{Slate}480,000 行}\\[.5mm]
    {\bfseries\scriptsize 密集关系探索}\\[.2mm]
    {\scriptsize\color{Slate}密集采样后还剩什么非线性关系？}
  };
  \node[scene card,fill=Coral!8] (c6) at (120,-144) {
    {\ttfamily\fsxs\bfseries\color{Coral} retention}\hfill{\fsxs\color{Slate}320,000 行}\\[.5mm]
  ...
  \node[scene card,fill=Viridian!8] (c7) at (4,-163) {
    {\ttfamily\fsxs\bfseries\color{Viridian} quality-matrix}\hfill{\fsxs\color{Slate}280,000 行}\\[.5mm]
    {\bfseries\scriptsize 质量风险矩阵}\\[.2mm]
    {\scriptsize\color{Slate}哪些阶段与子系统累积质量风险？}
  };
  \node[scene card,fill=Ocean!8] (c8) at (62,-163) {
    {\ttfamily\fsxs\bfseries\color{Ocean} rankings}\hfill{\fsxs\color{Slate}240,000 行}\\[.5mm]
    {\bfseries\scriptsize 区域绩效排名}\\[.2mm]
    {\scriptsize\color{Slate}区域评分与不确定性如何比较？}
  };
  \node[scene card,fill=Magma!8] (c9) at (120,-163) {
    {\ttfamily\fsxs\bfseries\color{Magma} anomalies}\hfill{\fsxs\color{Slate}420,000 行}\\[.5mm]
  ...
  \node[scene card,fill=Teal!8] (c10) at (4,-182) {
    {\ttfamily\fsxs\bfseries\color{Teal} vector-field}\hfill{\fsxs\color{Slate}180,000 行}\\[.5mm]
    {\bfseries\scriptsize 采样流场向量}\\[.2mm]
    {\scriptsize\color{Slate}方向流如何随采样网格变化？}
  };
  \node[scene card,fill=Violet!8] (c11) at (62,-182) {
    {\ttfamily\fsxs\bfseries\color{Violet} small-multiples}\hfill{\fsxs\color{Slate}360,000 行}\\[.5mm]
    {\bfseries\scriptsize 服务行为小倍数}\\[.2mm]
    {\scriptsize\color{Slate}同指标在四个服务中如何表现？}
  };
  \node[scene card,fill=Gold!12] (c12) at (120,-182) {
    {\ttfamily\fsxs\bfseries\color{Gold!70!black} operations}\hfill{\fsxs\color{Slate}520,000 行}\\[.5mm]
  ...
  \begin{scope}[shift={(8,-214)}]
    ...
    \def\gap{2.7}
    \def\maxH{50}
    % baseline (top); bars start 1mm below it so the axis stroke keeps clearance
    \draw[draw=Fog,line width=.6pt] (0,0) -- (158,0);
    % y-axis labels (top to bottom: 1M at top, 0 at bottom)
    \node[font=\rmfamily\fsxs,text=Slate,anchor=east] at (-2, -1) {1M};
    \node[font=\rmfamily\fsxs,text=Slate,anchor=east] at (-2, -26) {500K};
    \node[font=\rmfamily\fsxs,text=Slate,anchor=east] at (-2, -\maxH-1) {0};
    \node[font=\rmfamily\fsxs,text=Slate,anchor=east,rotate=90] at (-7, -26) {行数};
    % bars (draw downward: y=-1 to y=-1-h)
    \foreach \i/\h/\col/\lab in {
      ...
      \draw[fill=\col!45,draw=\col!60,line width=.4pt,rounded corners=.6mm]
        (\xpos, -1) rectangle (\xpos + \barW, -1-\h);
      \node[font=\ttfamily\fsxs,text=Slate,anchor=north,rotate=45,inner sep=.5pt]
        at (\xpos + \barW/2, -\maxH - 2.5) {\lab};
    }
  \end{scope}

  % Panel 2 bg
  \begin{pgfonlayer}{bg layer}
    \node[panel bg,fit={(k2.north west) (172,-274)},inner sep=0mm] (p2bg) {};
  \end{pgfonlayer}
  ...
  \node[pipe build,anchor=north west] (build) at (47.5,-300) {
    \textbf{Vite 构建}\\[.6mm]
    \footnotesize\texttt{vite build} $\to$ \texttt{dist/}
  };
  \node[pipe browser,anchor=north west] (pw) at (91,-300) {
    \textbf{Playwright 浏览器}\\[.6mm]
    \footnotesize 无头 Chromium + Chrome
  };
  \node[pipe output,anchor=north west] (out) at (134.5,-300) {
    \textbf{透明 PNG 输出}\\[.6mm]
    \footnotesize\texttt{out/}\\
    \footnotesize\texttt{*-transparent.png}
  };

  % Preview server below build
  \node[pipe build,anchor=north west,fill=white,draw=Ocean!40,text=Ocean!70!black] (preview) at (47.5,-318) {
    \textbf{预览服务器}\\[.6mm]
  ...
  \begin{pgfonlayer}{edge layer}
    ...
    \draw[primary flow] (build.east) -- (pw.west)
      node[edge label,midway,above=.3mm] {静态\\页面};
    \draw[primary flow] (pw.east) -- (out.west)
      node[edge label,midway,above=.3mm] {截图};
    \draw[data flow] (build.south) -- (preview.north)
      node[edge label,pos=0.35,right=1.5mm] {serve};
  \end{pgfonlayer}

  % Validation gates
  \node[panel kicker] (vk) at (4,-330) {三重像素级验证门};

  \node[gate node,anchor=north west] (g1) at (28.5,-338) {透明度\\校验};
  \node[gate node,anchor=north west] (g2) at (80.5,-338) {墨迹像素\\校验};
  \node[gate node,anchor=north west] (g3) at (132.5,-338) {色彩饱和\\度校验};

  % Gate descriptions
  \node[font=\rmfamily\scriptsize,text=Slate,text width=38mm,align=center,anchor=north] (gd1) at (36,-357) {
    透明像素 > 18\%\\确保背景真正透明
  };
  \node[font=\rmfamily\scriptsize,text=Slate,text width=38mm,align=center,anchor=north] (gd2) at (88,-357) {
    可见像素 > 8,000\\确保图形非空
  };
  \node[font=\rmfamily\scriptsize,text=Slate,text width=38mm,align=center,anchor=north] (gd3) at (140,-357) {
    饱和像素 > 1,200\\确保彩色数据标记
  };

  % Verify flow arrows (from output down to a bus lane, then to three gates)
  \begin{pgfonlayer}{edge layer}
    \draw[verify flow] (out.south) -- ++(0,-22mm) coordinate (vbranch)
      node[edge label,midway,right=1.5mm] {PNG 像素分析};
  ...
  % Panel 3 bg
  \begin{pgfonlayer}{bg layer}
    \node[panel bg,fit={(k3.north west) (172,-384)},inner sep=0mm] (p3bg) {};
  \end{pgfonlayer}
  ...
  \begin{pgfonlayer}{bg layer}
    \fill[Paper] (0,8) rectangle (176,-404);
  \end{pgfonlayer}
